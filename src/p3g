...
import os, base64, mimetypes, traceback, threading, glob
import gradio as gr
...
# ----------------------------
# Gradio UI (MODIFIED FOR DOCKER MODEL PATHS)
# ----------------------------
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Local VL · GGUF+mmproj · Streaming") as demo:
        gr.Markdown("### Local Multimodal LLM · Docker Version")

        # --- NEW CODE: Auto-discovery of models ---
        model_dir = "/app/models"
        
        # Check if we have models mounted
        # Scan for .gguf files
        found_models = sorted(glob.glob(f"{model_dir}/*.gguf")) if os.path.isdir(model_dir) else []
        
        with gr.Row():
            if found_models:
                # Docker Mode: Select from list
                # Note: We filter out files with 'mmproj' in the name so they don't clutter the main model list
                main_models = [m for m in found_models if "mmproj" not in os.path.basename(m).lower()]
                if not main_models: main_models = found_models # fallback if everything is named mmproj
                
                model_file = gr.Dropdown(
                    choices=main_models, 
                    value=main_models[0] if main_models else None, 
                    label=f"Select Model (Found {len(main_models)} in {model_dir})",
                    type="value"
                )
            else:
                # Fallback: Upload mode if folder is empty or not found
                model_file = gr.File(label="Model GGUF", file_types=[".gguf"], type="filepath")
            
            # Same logic for mmproj (vision clips)
            found_mmproj = sorted(glob.glob(f"{model_dir}/*mmproj*.gguf")) if os.path.isdir(model_dir) else []
            # Also try to find anything with 'clip' in the name just in case
            if not found_mmproj and os.path.isdir(model_dir):
                 found_mmproj = sorted(glob.glob(f"{model_dir}/*clip*.gguf"))

            if found_mmproj:
                 mmproj_file = gr.Dropdown(choices=[""] + found_mmproj, label="mmproj (Vision Adapter)", type="value")
            else:
                 mmproj_file = gr.File(label="mmproj GGUF (optional)", file_types=[".gguf"], type="filepath")
        # ------------------------------------------
        ...
        def _on_load(model_path, mmproj_path, handler, cf_choice, ctx, ngpu, verb):
            ...
            # Handle Dropdown returning None/empty string for optional selection
            if not mmproj_path: 
                mmproj_path = None
